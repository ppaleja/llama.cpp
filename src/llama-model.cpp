        ...
        case LLM_ARCH_QWEN3NEXT:
            {
                ...
                for (uint32_t i = 0; i < hparams.n_layer; ++i) {
                    hparams.recurrent_layer_arr[i] =
                        ((i + 1) % 4 != 0);  // TODO: extract the magic 4 from "full_attention_interval"
                }
            ...
            case LLM_ARCH_MOTIF:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        layer.wq =
                            create_tensor(tn(LLM_TENSOR_ATTN_Q, "weight", i), { n_embd, n_embd_head_k * n_head }, 0);
                        layer.wk =
                            create_tensor(tn(LLM_TENSOR_ATTN_K, "weight", i), { n_embd, n_embd_head_k * n_head_kv }, 0);
                        layer.wv = create_tensor(tn(LLM_TENSOR_ATTN_V, "weight", i), { n_embd, n_embd_v_gqa }, 0);
                        ...
                        layer.wo = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i),
                                                 { (int64_t) (4 * n_embd_v_gqa), n_embd }, 0);
                        ...
                        layer.attn_sub_norm =
                            create_tensor(tn(LLM_TENSOR_ATTN_SUB_NORM, "weight", i), { 2 * n_embd_head_k }, 0);
                        layer.ffn_norm   = create_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), { n_embd }, 0);
                        layer.rope_freqs = create_tensor(tn(LLM_TENSOR_ROPE_FREQS, "weight", i), { n_rot / 2 },
                                                         TENSOR_NOT_REQUIRED | (i != 0 ? TENSOR_DUPLICATED : 0));
